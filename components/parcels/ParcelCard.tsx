...
import { Package, MapPin, User, Phone, Calendar, Truck } from "lucide-react"
...
export function ParcelCard({ parcel, onTrack, onUpdateStatus, showActions = true }: ParcelCardProps) {
    ...
    <Card className="w-full">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge className={STATUS_COLORS[parcel.status as keyof typeof STATUS_COLORS]}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          ...
          <div>
            <span className="font-medium">Payment:</span> {parcel.paymentType.toUpperCase()}
            {parcel.paymentType === "cod" && parcel.codAmount && ` (₹${parcel.codAmount})`}
          </div>
        </div>
        ...
        {showActions && (
          <div className="flex gap-2 pt-2">
            {onTrack && (
              <Button variant="outline" size="sm" onClick={() => onTrack(parcel)}>
                Track
              </Button>
            )}
            {onUpdateStatus && (
              <Button variant="outline" size="sm" onClick={() => onUpdateStatus(parcel)}>
                Update Status
              </Button>
            )}
          </div>
